...
---------------
    ...
    3. call_llm       — Anthropic claude-opus-4-7 (1M context)
...
for noisy in ("httpx", "anthropic", "urllib3"):
...
class Config:
    ...
    model:     str  = field(default_factory=lambda: os.getenv("ANTHROPIC_MODEL", "claude-opus-4-7"))
    max_toks:  int  = field(default_factory=lambda: int(os.getenv("ANTHROPIC_MAX_TOKENS", "12000")))
    ...
    def validate(self, *, full: bool = True) -> list[str]:
        ...
        if full:
            if not self.api_key:
                errs.append("ANTHROPIC_API_KEY missing (set in seo-pipeline/.env)")
